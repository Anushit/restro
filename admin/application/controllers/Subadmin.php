...
class Subadmin extends MY_Controller {
	...
	public function datatable_json(){				   					   
		...
		{  
			...
			$action = "";
			if(@$this->general_user_premissions['subadmin']['is_edit']==1){
				$action .= '<a title="Edit" class="update btn btn-sm btn-warning" href="'.base_url('subadmin/edit/'.$row['admin_id']).'"> <i class="fa fa-pencil-square-o"></i></a>';
			}
			if(@$this->general_user_premissions['subadmin']['is_delete']==1){
				$action .= '<a title="Delete" class="delete btn btn-sm btn-danger" href='.base_url("subadmin/delete/".$row['admin_id']).' title="Delete" onclick="return confirm(\'Do you want to delete ?\')"> <i class="fa fa-trash-o"></i></a>';
			}
			if(@$this->general_user_premissions['subadmin']['is_view']==1){
				$action .= '<a title="Manage Permission" class="update btn btn-sm btn-warning" href="'.base_url('subadmin/manage_permission/'.$row['admin_id']).'"> <i class="fa fa-lock"></i></a>';
			}
	...
	public function add(){
		...
		if($this->input->post('submit')){
			$this->form_validation->set_rules('username', 'Username', 'trim|strip_tags|xss_clean|required|is_unique[ci_admin.username]');
			$this->form_validation->set_rules('firstname', 'Firstname', 'trim|strip_tags|xss_clean|required');
			$this->form_validation->set_rules('lastname', 'Lastname', 'trim|strip_tags|xss_clean|required');
			$this->form_validation->set_rules('email', 'Email', 'trim|strip_tags|xss_clean|valid_email|required|is_unique[ci_admin.email]');
			$this->form_validation->set_rules('mobile_no', 'Number', 'trim|required');
			$this->form_validation->set_rules('password', 'Password', 'trim|strip_tags|xss_clean|required');
			$this->form_validation->set_rules('role', 'Role', 'trim|required');
			$this->form_validation->set_rules('conf_password', 'Confirm Password', 'trim|strip_tags|xss_clean|required|matches[password]');
	...
	public function edit($id = 0){
		...
		if($this->input->post('submit')){
			...
			$this->form_validation->set_rules('username', 'Username', 'trim|strip_tags|required|xss_clean'.$uis_unique);
			$this->form_validation->set_rules('firstname', 'Username', 'trim|strip_tags|xss_clean|required');
			$this->form_validation->set_rules('lastname', 'Lastname', 'trim|strip_tags|xss_clean|required');
			$this->form_validation->set_rules('email', 'Email', 'trim|strip_tags|xss_clean|valid_email|required'.$eis_unique);
			$this->form_validation->set_rules('mobile_no', 'Number', 'trim|required');
	...
	{		
		...
		$this->db->update('ci_admin', array('deleted'=>1,'is_active'=>0));
	...
	public function manage_permission($id){  
		...
		$module = $this->role_model->get_allmodel();
        ...
        $i = 1;             
            foreach ($permission as $rs) {
                ...
                $data.=' <td class="text-center">';
                 if($modules[$rs->module_id]['is_add']==1){ 
                 	$data.= '<input type="checkbox" class="form-check-input mt-0 pt-0" id="is_add' . $rs->module_id.$rs->id.'" data-modelid="'. $rs->id.'"data-userid="'. $rs->permission_id.'" data-type="is_add" onChange="actions(this.id)" ';if($rs->is_allow == 0){ $data.=  'disabled' ;} if($rs->is_add == 1){  $data.=  'checked' ; }
                        $data.=' >';}
                        $data.= '</td>';

                $data.=' <td class="text-center">';
                if($modules[$rs->module_id]['is_view']==1){
                	 $data.=' <input type="checkbox" class="form-check-input mt-0 pt-0" id="is_view' . $rs->module_id.$rs->id.'" data-modelid="'. $rs->id.'"data-userid="'. $rs->permission_id.'" data-type="is_view" onChange="actions(this.id)" ';if($rs->is_allow == 0){ $data.=  'disabled'; } if($rs->is_view == 1){   $data.=  'checked' ;}
                     $data.=' >';}
                     $data.= '</td>';

					$data.=' <td class="text-center">';
					if($modules[$rs->module_id]['is_edit']==1){
					$data.='<input type="checkbox" class="form-check-input mt-0 pt-0" id="is_edit' . $rs->module_id.$rs->id.'" data-modelid="'. $rs->id.'"data-userid="'. $rs->permission_id.'" data-type="is_edit" onChange="actions(this.id)" ';if($rs->is_allow == 0){ $data.=  'disabled'; } if($rs->is_edit == 1){  $data.=  'checked' ; }
                     $data.=' >';}
                     $data.= '</td>';
                $data.=' <td class="text-center">';
                if($modules[$rs->module_id]['is_delete']==1){
                	$data.='<input type="checkbox" class="form-check-input mt-0 pt-0" id="is_delete' . $rs->module_id.$rs->id.'" data-modelid="'. $rs->id.'"data-userid="'. $rs->permission_id.'" data-type="is_delete"onChange="actions(this.id)" ';if($rs->is_allow == 0){ $data.=  'disabled'; } if($rs->is_delete == 1){  $data.=  'checked' ;}
                    $data.=' >';}
                    $data.= '</td>';
