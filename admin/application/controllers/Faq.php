...
class Faq extends MY_Controller {
	...
	public function datatable_json(){				   					   
		...
		{  
			...
			$action = "";
			if(@$this->general_user_premissions['faq']['is_edit']==1){
				$action .= '<a title="Edit" class="update btn btn-sm btn-warning" href="'.base_url('Faq/edit/'.$row['id']).'"> <i class="fa fa-pencil-square-o"></i></a>';
			}
			if(@$this->general_user_premissions['faq']['is_delete']==1){
				$action .= '<a title="Delete" class="delete btn btn-sm btn-danger" href='.base_url("Faq/delete/".$row['id']).' title="Delete" onclick="return confirm(\'Do you want to delete ?\')"> <i class="fa fa-trash-o"></i></a>';
			}
			$data[]= array(
				++$i,
				$row['faq_name'],
	...
	public function add(){
		...
		if($this->input->post('submit')){
			$this->form_validation->set_rules('faqtype', 'FAQ Type', 'trim|strip_tags|xss_clean|required');
			$this->form_validation->set_rules('answer', 'Answer ', 'trim|strip_tags|xss_clean|required');
			$this->form_validation->set_rules('faq_question', 'Question', 'trim|strip_tags|xss_clean|required');
	...
	{	
		$this->db->where('id', $id);
		$this->db->update('ci_faq', array('deleted'=>1,'is_active'=>0));
