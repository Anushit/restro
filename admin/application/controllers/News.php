...
class News extends MY_Controller {
	...
	public function datatable_json(){				   					   
		...
		{  
			...
			$action = "";
			if(@$this->general_user_premissions['news']['is_edit']==1){
				$action .= '<a title="Edit" class="update btn btn-sm btn-warning" href="'.base_url('news/edit/'.$row['id']).'"> <i class="fa fa-pencil-square-o"></i></a>';
			}
			if(@$this->general_user_premissions['news']['is_delete']==1){
				$action .= '<a title="Delete" class="delete btn btn-sm btn-danger" href='.base_url("news/delete/".$row['id']).' title="Delete" onclick="return confirm(\'Do you want to delete ?\')"> <i class="fa fa-trash-o"></i></a>';
			}
			$data[]= array(
				++$i,
				$row['name'],  
...
//-----------------------------------------------------------
	public function add(){
		...
		if($this->input->post('submit')){
			$this->form_validation->set_rules('name', 'Name', 'trim|required|strip_tags|xss_clean|is_unique[ci_newsupdates.name]');
			$this->form_validation->set_rules('sort_description', 'Sort Description', 'trim|strip_tags|xss_clean|required');
			$this->form_validation->set_rules('description', 'Description', 'trim|strip_tags|xss_clean|required');
			$this->form_validation->set_rules('news_date', 'News_Date', 'trim|strip_tags|xss_clean|required');
			$this->form_validation->set_rules('news_location', 'News Location', 'trim|strip_tags|xss_clean|required');
			$this->form_validation->set_rules('news_time', 'News Time', 'trim|strip_tags|xss_clean|required'); 
			$this->form_validation->set_rules('status', 'Status', 'trim|strip_tags|xss_clean|required');
			$this->form_validation->set_rules('slug', 'Seo URL', 'trim|required|strip_tags|xss_clean|is_unique[ci_newsupdates.slug]');
...
public function edit($id = 0){
		...
		if($this->input->post('submit')){
			$original_valuename = $this->news_model->get_news_by_id($id);
			if($this->input->post('name') != $original_valuename['name']) {
			   $uis_unique =  '|is_unique[ci_newsupdates.name]';
			} else {
			   $uis_unique =  '';
			}
			$olddata = $this->product_model->get_news_by_id($id);
			if($this->input->post('slug') != $olddata['slug']) {
			   $uis_unique =  '|is_unique[ci_newsupdates.slug]';
			} else {
			   $uis_uniqueslug =  '';
			}
			...
			$this->form_validation->set_rules('news_time', 'News Time', 'trim|strip_tags|xss_clean|required'); 
			$this->form_validation->set_rules('status', 'Status', 'trim|strip_tags|xss_clean|required');
			$this->form_validation->set_rules('slug', 'Seo URL', 'trim|strip_tags|xss_clean|required'.$uis_uniqueslug);
	...
	{
		$this->db->where('id', $id);
		$this->db->update('ci_newsupdates', array('deleted'=>1,'is_active'=>0));
