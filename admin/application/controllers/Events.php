...
class Events extends MY_Controller {
	...
	public function datatable_json(){				   					   
		...
		{  
			...
			$action = "";
			if(@$this->general_user_premissions['events']['is_edit']==1){
				$action .= '<a title="Edit" class="update btn btn-sm btn-warning" href="'.base_url('events/edit/'.$row['id']).'"> <i class="fa fa-pencil-square-o"></i></a>';
			}
			if(@$this->general_user_premissions['events']['is_delete']==1){
				$action .= '<a title="Delete" class="delete btn btn-sm btn-danger" href='.base_url("events/delete/".$row['id']).' title="Delete" onclick="return confirm(\'Do you want to delete ?\')"> <i class="fa fa-trash-o"></i></a>';
			}
			$data[]= array(
				++$i,
				$row['name'], 
...
//-----------------------------------------------------------
	public function add(){
		...
		if($this->input->post('submit')){
			$this->form_validation->set_rules('name', 'Name', 'trim|required|strip_tags|xss_clean|is_unique[ci_events.name]');
			$this->form_validation->set_rules('sort_description', 'Sort Description', 'trim|strip_tags|xss_clean|required');
			$this->form_validation->set_rules('event_date', 'Event_Date', 'trim|strip_tags|xss_clean|required');
			$this->form_validation->set_rules('event_location', 'Event Location', 'trim|strip_tags|xss_clean|required');
			$this->form_validation->set_rules('event_start_time', 'Event Start Time', 'trim|strip_tags|xss_clean|required');
			$this->form_validation->set_rules('event_end_time', 'Event End Time', 'trim|strip_tags|xss_clean|required');
			$this->form_validation->set_rules('status', 'Status', 'trim|strip_tags|xss_clean|required');
			$this->form_validation->set_rules('slug', 'Seo URL', 'trim|required|strip_tags|xss_clean|is_unique[ci_events.slug]');
...
public function edit($id = 0){
		...
		if($this->input->post('submit')){
			...
			$this->form_validation->set_rules('name', 'Name', 'trim|required|strip_tags|xss_clean'.$uis_unique); 
			$this->form_validation->set_rules('sort_description', 'Sort Description', 'trim|strip_tags|xss_clean|required');
	...
	{
		$this->db->where('id', $id);
		$this->db->update('ci_events', array('deleted'=>1,'is_active'=>0));
