...
class Testimonial extends MY_Controller {
	...
	public function datatable_json(){				   					   
		...
		{  
			...
			$action = "";
			if(@$this->general_user_premissions['testimonial']['is_edit']==1){
				$action .= '<a title="Edit" class="update btn btn-sm btn-warning" href="'.base_url('testimonial/edit/'.$row['id']).'"> <i class="fa fa-pencil-square-o"></i></a>';
			}
			if(@$this->general_user_premissions['testimonial']['is_delete']==1){
				$action .= '<a title="Delete" class="delete btn btn-sm btn-danger" href='.base_url("testimonial/delete/".$row['id']).' title="Delete" onclick="return confirm(\'Do you want to delete ?\')"> <i class="fa fa-trash-o"></i></a>';
			}
			$data[]= array(
				++$i,
				$row['name'], 
	...
	public function add(){
		...
		if($this->input->post('submit')){
			...
			$this->form_validation->set_rules('designation', 'Designation', 'trim|strip_tags|xss_clean|required'); 
	...
	{	
		$this->db->where('id', $id);
		$this->db->update('ci_testimonials', array('deleted'=>1,'is_active'=>0));
