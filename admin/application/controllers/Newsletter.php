...
class Newsletter extends MY_Controller {
	...
	public function datatable_json(){				   					   
		...
		{  
			...
			$action = "";
			if(@$this->general_user_premissions['newsletter']['is_delete']==1){
				$action .= '<a title="Delete" class="delete btn btn-sm btn-danger" href='.base_url("newsletter/delete/".$row['id']).' title="Delete" onclick="return confirm(\'Do you want to delete ?\')"> <i class="fa fa-trash-o"></i></a>';
			}
			$data[]= array(
				++$i,
				$row['email'],
	...
	public function send_mail(){
		...
		if($this->input->post('submit')){
			...
			$this->form_validation->set_rules('users', 'User', 'trim|strip_tags|xss_clean|required');
