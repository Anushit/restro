...
class Partner extends MY_Controller {
	...
	public function datatable_json(){				   					   
		...
		{  
			...
			$action = "";
			if(@$this->general_user_premissions['partner']['is_edit']==1){
				$action .= '<a title="Edit" class="update btn btn-sm btn-warning" href="'.base_url('partner/edit/'.$row['id']).'"> <i class="fa fa-pencil-square-o"></i></a>';
			}
			if(@$this->general_user_premissions['partner']['is_delete']==1){
				$action .= '<a title="Delete" class="delete btn btn-sm btn-danger" href='.base_url("partner/delete/".$row['id']).' title="Delete" onclick="return confirm(\'Do you want to delete ?\')"> <i class="fa fa-trash-o"></i></a>';
			}
			$data[]= array(
				++$i,
				$row['name'],  
	...
	public function add(){
		...
		if($this->input->post('submit')){
			...
			$this->form_validation->set_rules('mobile', 'Mobile', 'trim|numeric|strip_tags|xss_clean|required'); 
			...
			$this->form_validation->set_rules('slug', 'Seo URL', 'trim|required|strip_tags|xss_clean|is_unique[ci_partner.slug]');
	...
	public function edit($id = 0){
		...
		if($this->input->post('submit')){
			$olddata = $this->partner_model->get_partner_by_id($id);
			if($this->input->post('slug') != $olddata['slug']) {
			   $uis_unique =  '|is_unique[strip_tags|xss_clean|ci_products.slug]';
			} else {
			   $uis_unique =  '';
			}
			$this->form_validation->set_rules('name', 'Name', 'trim|strip_tags|xss_clean|required');
			$this->form_validation->set_rules('email', 'Email', 'trim|valid_email|strip_tags|xss_clean|required');
			$this->form_validation->set_rules('mobile', 'Mobile', 'trim|numeric|strip_tags|xss_clean|required'); 
			$this->form_validation->set_rules('sort_order', 'Sort Order', 'trim|numeric|strip_tags|xss_clean|required');
			$this->form_validation->set_rules('status', 'Status', 'trim|strip_tags|xss_clean|required');
	...
	{		
		$this->db->where('id', $id);
		$this->db->update('ci_partners', array('deleted'=>1,'is_active'=>0));
