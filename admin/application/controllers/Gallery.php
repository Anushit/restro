...
class Gallery extends MY_Controller {
	...
	public function photo_datatable_json(){				   					   
		...
		{  
			...
			$action = "";
			if(@$this->general_user_premissions['photo_gallery']['is_edit']==1){
				$action .= '<a title="Edit" class="update btn btn-sm btn-warning" href="'.base_url('gallery/editphoto/'.$row['id']).'"> <i class="fa fa-pencil-square-o"></i></a> &nbsp;';
			}
			if(@$this->general_user_premissions['photo_gallery']['is_delete']==1){
				$action .= '<a title="Delete" class="delete btn btn-sm btn-danger" href='.base_url("gallery/deletephoto/".$row['id']).' title="Delete" onclick="return confirm(\'Do you want to delete ?\')"> <i class="fa fa-trash-o"></i></a>';
			}
			$data[]= array(
				++$i,
				$row['album'],
				...
				'<a title="Add More Photos" style="margin-bottom:5px;" class="update btn btn-sm btn-info" href="'.base_url('gallery/updatephoto/'.$row['id']).'"> <i class="fa fa-plus">&nbsp;</i> Add Photos</a> <br>'.$action				 
			);
		}
		$records['data']=$data;
	...
	public function video_datatable_json(){				   					   
		...
		{  
			$status = ($row['is_active'] == 1)? 'checked': '';
			$action = "";
			if(@$this->general_user_premissions['video_gallery']['is_edit']==1){
				$action .= '<a title="Edit" class="update btn btn-sm btn-warning" href="'.base_url('gallery/editvideo/'.$row['id']).'"> <i class="fa fa-pencil-square-o"></i></a> &nbsp;';
			}
			if(@$this->general_user_premissions['video_gallery']['is_delete']==1){
				$action .= '<a title="Delete" class="delete btn btn-sm btn-danger" href='.base_url("gallery/deletevideo/".$row['id']).' title="Delete" onclick="return confirm(\'Do you want to delete ?\')"> <i class="fa fa-trash-o"></i></a>';
			}
			$data[]= array(
				++$i,
				$row['album'],
				...
				'<a title="Add More Video" style="margin-bottom:5px;" class="update btn btn-sm btn-info" href="'.base_url('gallery/updatevideo/'.$row['id']).'"> <i class="fa fa-plus">&nbsp;</i> Add Video</a> <br>'.$action
	...
	public function addphoto(){
		...
		if($this->input->post('submit')){
			$this->form_validation->set_rules('album', 'Album', 'trim|strip_tags|xss_clean|required');
			$this->form_validation->set_rules('sort_order', 'Sort Order', 'trim|numeric|strip_tags|xss_clean|required');
			  
			$this->form_validation->set_rules('status', 'Status', 'trim|strip_tags|xss_clean|required');
			$this->form_validation->set_rules('slug', 'Seo URL', 'trim|required|strip_tags|xss_clean|is_unique[ci_gallery.slug]');
	...
	public function editphoto($id = 0){
		if($this->input->post('submit')){
			$olddata = $this->gallery_model->get_gallery_by_id($id);
			if($this->input->post('slug') != $olddata['slug']) {
			   $uis_unique =  '|is_unique[ci_gallery.slug]';
			} else {
			   $uis_unique =  '';
			}
			$this->form_validation->set_rules('album', 'Album', 'trim|strip_tags|xss_clean|required');
	...
	{		
		...
		$this->db->update('ci_gallery', array('deleted'=>1,'is_active'=>0));
		...
		$this->db->update('ci_gallery_details', array('deleted'=>1,'is_active'=>0));
	...
	public function updatevideo($id = 0){
		...
		if($this->input->post('submit')){   
				$oldfile = [];
				 $oldvideo = $this->input->post('old_image');
				// $newvideo =$this->input->post('video_url');
				foreach ($oldvideo as $key => $value) {
				 $this->db->delete('ci_gallery_details', array('gallery_id' => $id,'value'!= $value['value']));
				}
