...
class Role extends MY_Controller {
	...
	public function datatable_json(){				   					   
		...
		{  
			...
			$action = "";
			if(@$this->general_user_premissions['role']['is_edit']==1){
				$action .= '<a title="Manage Permission" class="update btn btn-sm btn-warning" href="'.base_url('role/manage_parmission/'.$row['id']).'"> <i class="fa fa-lock"></i></a>';
			}
			if(@$this->general_user_premissions['role']['is_delete']==1){
				$action .= '<a title="Delete" class="delete btn btn-sm btn-danger" href='.base_url("role/delete/".$row['id']).' title="Delete" onclick="return confirm(\'Do you want to delete ?\')"> <i class="fa fa-trash-o"></i></a>';
			}
			$data[]= array(
				++$i,
				$row['name'],
	...
	public function add(){
		...
		if($this->input->post('submit')){
			$this->form_validation->set_rules('name', 'role Name', 'trim|required|strip_tags|xss_clean|is_unique[ci_role.name]');
	...
	{		
		$this->db->where('id', $id);
		$this->db->update('ci_role', array('deleted'=>1,'is_active'=>0));
