...
class Whatsapp extends MY_Controller {
	...
	public function index(){
		...
		$data['title'] = 'Whatsapp Msg List';
		
		$this->load->view('includes/_header', $data);
		$this->load->view('whatsapp/sms_list');
		$this->load->view('includes/_footer');
	}
	
	public function datatable_json(){				   					   
		$records = $this->message_model->get_whatsapp_list();
		$data = array();
	...
	public function add(){
		if($this->input->post('submit')){
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$this->load->view('whatsapp/send_sms', $data);
				$this->load->view('includes/_footer');
			}
			else{
				...
				$result = $this->message_model->add_message($data);
					...
					if($result){
						...
						redirect(base_url('whatsapp'));
					}else{
						$this->session->set_flashdata('error', 'Uanable To send whatsapp msg somthing Want to wrong');
				  		redirect(base_url('whatsapp/send_sms'), 'refresh');
					}
			...
			$this->load->view('includes/_header', $data);
			$this->load->view('whatsapp/send_sms');
			$this->load->view('includes/_footer');
		}
	...
	public function getmemberid(){
		...
		foreach ($records as  $value) {
			$data.=  '<div class="col-md-12"><i class="fa fa-whatsapp"></i>&nbsp' .$value["reciver"].'</div>';
		}
