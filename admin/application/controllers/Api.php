...
public function productlist_post(){  
    ...
    public function menuinquery_post(){ 
        if ($this->input->server('REQUEST_METHOD') == 'POST') 
        {   
           $linkID = ($this->input->post('link_id'))?$this->input->post('link_id'):0;
            $data = array(
    ...
    public function all_products_post(){
        ...
        {      
            ...
            $limit = !empty($this->post('per_page'))?$this->post('per_page'):10;
            $page = !empty($this->post('page'))?$this->post('page'):1;
            $start = ($page-1)*$limit;
          
            $product_data = $this->product_model->all_products($cat_id, $search, $start, $limit);

            $product_count = $this->product_model->get_all_products_count($cat_id, $search);
    ...
    public function product_details_get($id=null){
        if ($this->input->server('REQUEST_METHOD') == 'GET') 
        {      
            $product__details_data = $this->product_model->get_products_by_id($id);
