...
class Portfolio extends MY_Controller {
	...
	public function datatable_json(){				   					   
		...
		{  
			...
			$action = "";
			if(@$this->general_user_premissions['portfolio']['is_edit']==1){
				$action .= '<a title="Edit" class="update btn btn-sm btn-warning" href="'.base_url('portfolio/edit/'.$row['id']).'"> <i class="fa fa-pencil-square-o"></i></a>';
			}
			if(@$this->general_user_premissions['portfolio']['is_delete']==1){
				$action .= '<a title="Delete" class="delete btn btn-sm btn-danger" href='.base_url("portfolio/delete/".$row['id']).' title="Delete" onclick="return confirm(\'Do you want to delete ?\')"> <i class="fa fa-trash-o"></i></a>';
			}
			$data[]= array(
				++$i,
				$row['name'],
	...
	public function add(){
		...
		if($this->input->post('submit')){
			$this->form_validation->set_rules('name', 'Name', 'trim|strip_tags|strip_tags|xss_clean|required');
			...
			$this->form_validation->set_rules('slug', 'Seo Url', 'trim|required|strip_tags|xss_clean|is_unique[ci_portfolio.slug]');
	...
	public function edit($id = 0){
		...
		if($this->input->post('slug') != $checkid['slug']) {
			   $uis_unique =  '|is_unique[ci_portfolio.slug]';
			} else {
			   $uis_unique =  '';
			}
		if (empty($checkid)) {
	...
	public function export_csv(){ 
	   ...
	   // file creation 
		...
		$header = array("ID", "Name", "Sort description", "Description","feature","Sort Order","meta_title", "Created Date","created_by"); 
