...
class Service extends MY_Controller {
	...
	public function datatable_json(){				   					   
		...
		{  
			...
			$action = "";
			if(@$this->general_user_premissions['service']['is_edit']==1){
				$action .= '<a title="Edit" class="update btn btn-sm btn-warning" href="'.base_url('service/edit/'.$row['id']).'"> <i class="fa fa-pencil-square-o"></i></a>';
			}
			if(@$this->general_user_premissions['service']['is_delete']==1){
				$action .= '<a title="Delete" class="delete btn btn-sm btn-danger" href='.base_url("service/delete/".$row['id']).' title="Delete" onclick="return confirm(\'Do you want to delete ?\')"> <i class="fa fa-trash-o"></i></a>';
			}
			$data[]= array(
				++$i,
				$row['name'],
	...
	public function edit($id = 0){
		...
		if($this->input->post('submit')){
			$olddata = $this->service_model->get_service_by_id($id);
			if($this->input->post('slug') != $olddata['slug']) {
			   $uis_unique =  '|is_unique[ci_services.slug]';
			} else {
			   $uis_unique =  '';
			}
			$this->form_validation->set_rules('name', 'Name', 'trim|strip_tags|xss_clean|required');
