...
class Blogs extends MY_Controller {
	...
	public function datatable_json(){				   					   
		...
		{  
			...
			$action = "";
			if(@$this->general_user_premissions['blogs']['is_edit']==1){
				$action = '<a title="Edit" class="update btn btn-sm btn-warning" href="'.base_url('blogs/edit/'.$row['id']).'"> <i class="fa fa-pencil-square-o"></i></a>';
			}
			if(@$this->general_user_premissions['blogs']['is_delete']==1){
				$action .= '<a title="Delete" class="delete btn btn-sm btn-danger" href='.base_url("blogs/delete/".$row['id']).' title="Delete" onclick="return confirm(\'Do you want to delete ?\')"> <i class="fa fa-trash-o"></i></a>';
			}
...
//-----------------------------------------------------------
	public function add(){
		...
		if($this->input->post('submit')){
			$this->form_validation->set_rules('name', 'Name', 'trim|required|strip_tags|xss_clean|is_unique[ci_blogs.name]');
			...
			$this->form_validation->set_rules('blog_date', 'Blog Date', 'trim|strip_tags|xss_clean|required');
			$this->form_validation->set_rules('posted_by', 'Posted By', 'trim|strip_tags|xss_clean|required'); 
			$this->form_validation->set_rules('status', 'Status', 'trim|required');
			$this->form_validation->set_rules('slug', 'Seo URL', 'trim|required|strip_tags|xss_clean|is_unique[ci_blogs.slug]');
...
public function edit($id = 0){
		...
		if($this->input->post('submit')){
			...
			$this->form_validation->set_rules('description', 'Description', 'trim|strip_tags|xss_clean|required');
			$this->form_validation->set_rules('blog_date', 'Blog_Date', 'trim|strip_tags|xss_clean|required');
			$this->form_validation->set_rules('posted_by', 'Posted By', 'trim|strip_tags|xss_clean|required'); 
			$this->form_validation->set_rules('status', 'Status', 'trim|required');
	...
	public function delete($id = 0)
	{
		...
		$this->db->where('id', $id);
		$this->db->update('ci_blogs', array('deleted'=>1,'is_active'=>0));
