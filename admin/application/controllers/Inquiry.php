...
class Inquiry extends MY_Controller {
	...
	public function datatable_json(){	
		...
		if(!empty($_REQUEST['state'])){
			$where[] = " ci_inquiry.state= '".$_REQUEST['state']."'"; 
		}
		if(!empty($_REQUEST['country'])){
			$where[] = "ci_inquiry.country = '".$_REQUEST['country']."'"; 
		}
		if(!empty($_REQUEST['followup_date'])){
			$where[] = "ci_inquiry_followdetail.next_followup_date = '".$_REQUEST['followup_date']."'"; 
		}
		...
		{  
			...
			if(@$this->general_user_premissions['inquiry']['is_view']==1){
				$action .= '<a title="Manage Inquiry" class="delete btn btn-sm btn-info" href="'.base_url('inquiry/manage_inquiry_page/'.$row['id']).'"> <i class="fa fa-tasks"></i></a>';
			}
	...
	public function add(){
		$data['country'] = $this->inquiry_model->getallcountry();
		$data['referral'] = $this->inquiry_model->getallreferral();
