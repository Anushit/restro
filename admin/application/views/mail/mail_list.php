    ...
    <div class="card">
      <div class="card-header">
        <div class="d-inline-block">
          <h3 class="card-title"><i class="fa fa-list"></i>&nbsp; Mail List </h3>
        </div>
        <?php if(@$this->general_user_premissions['mail']['is_add']==1){ ?>
        <div class="d-inline-block float-right">
          <a href="<?= base_url('mail/add'); ?>" class="btn btn-success"><i class="fa fa-plus"></i> Add New Mail</a>
        </div>
      <?php } ?>
      </div>
    </div>
...
<script>
  ...
  var table = $('#na_datatable').DataTable( {
    "processing": true,
    "serverSide": true,
    "ajax": "<?=base_url('mail/datatable_json')?>",
    "order": [[5,'desc']],
    "columnDefs": [
    ...
    { "targets": 4, "name": "created_by", 'searchable':false, 'orderable':false},
   ...
   function member(id)
    { 
      ...
      var vdata = {
        ...
        $.ajax({
            type: "POST",
            url: "<?php echo base_url(); ?>mail/getmemberid",
            data: vdata,
