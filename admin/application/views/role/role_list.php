    ...
    <div class="card">
      <div class="card-header">
        <div class="d-inline-block">
          <h3 class="card-title"><i class="fa fa-list"></i>&nbsp; Role List </h3>
        </div>
        <div class="d-inline-block float-right">
         <?php if(@$this->general_user_premissions['role']['is_add']==1){ ?>
          <a href="<?= base_url('role/add'); ?>" class="btn btn-success"><i class="fa fa-plus"></i> Add New Role</a>
        <?php } ?>
        </div>
      </div>
    </div>
...
<script>
  ...
  var table = $('#na_datatable').DataTable( {
    "processing": true,
    "serverSide": true,
    "ajax": "<?=base_url('role/datatable_json')?>",
    "order": [[0,'desc']],
    "columnDefs": [
...
<script type="text/javascript">
  $("body").on("change",".tgl_checkbox",function(){
    console.log('checked');
    $.post('<?=base_url("role/change_status")?>',
    {
