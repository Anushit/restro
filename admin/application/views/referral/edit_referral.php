      ...
      <div class="card card-default color-palette-bo">
        ...
        <div class="card-body">   
           ...
           <!-- For Messages -->
            ...
            <?php echo form_open(base_url('referral/edit/'.$referral['id']), 'class="form-horizontal" id="inquiryForm"');  ?> 
              <div class="form-group row">                
                <div class="col-sm-6">
                  <label for="name" class="col-sm-6 control-label"> Name <span class="red">*</span></label>
                  <input type="text" name="name" class="form-control" id="name" placeholder="" value="<?= set_value('name',$referral['name']); ?>">
                </div>
                <div class="col-sm-6">
                  <label for="firmname" class="col-sm-6 control-label"> Firmname <span class="red">*</span></label>
                  <input type="text" name="firmname" class="form-control" id="firmname" placeholder="Enter Firmname" value="<?= set_value('firmname',$referral['firmname']); ?>">
                </div>
              </div>            
              <div class="form-group row">
               <div class="col-sm-6">
                  <label for="address" class="col-sm-6 control-label">Address <span class="red">*</span></label>
                  <input type="text" name="address" class="form-control" id="address" placeholder="" value="<?= set_value('address',$referral['address']); ?>">
                </div>
                <div class="col-sm-6">
                   <label for="phone" class="col-sm-6 control-label">Mobile No <span class="red">*</span></label>
                  <input type="text" name="phone" class="form-control" id="phone" placeholder="Enter Your 10 Digits Mobilbe Number" onkeyup="if (/\D/g.test(this.value)) this.value = this.value.replace(/\D/g, '')" value="<?= set_value('phone',$referral['phone']); ?>">
                </div> 
              </div>
              <div class="form-group row">
                <div class="col-sm-6">
                  ...
                  <select name="country" id="country" onchange ="get_state(this)" class="form-control">
                    ...
                    <?php foreach ($country as $key => $value) {?>
                     <option value="<?= $value['id']?>" <?= ($referral['country']==$value['id']) ? 'selected':''; ?>><?= $value['name']?>
                    </option>
                   <?php  } ?>
                  </select>
                </div>
                <div class="col-sm-6">
                  ...
                  <select name="state" id="state" onchange="get_city(this)"class="form-control">
                    ...
                    <?php foreach ($state as $key => $value) {?>
                     <option value="<?= $value['id']?>" <?= ($referral['state']==$value['id']) ? 'selected':''; ?>><?= $value['name']?>
                    </option>
                   <?php  } ?>
                  </select>
                </div>
                 ...
                 <label for="city" class="col-sm-6 control-label">City <span class="red">*</span></label>  
                   <select name="city_id" id="city_id" class="form-control">
                    ...
                    <?php foreach ($city as $key => $value) {?>
                     <option value="<?= $value['id']?>" <?= ($referral['city']==$value['id']) ? 'selected':''; ?>><?= $value['name']?>
                    </option>
                   <?php  } ?>
                  </select>
                </div>               
              ...
              <div class="form-group">
                <div class="col-md-12">
                  <input type="submit" name="submit" value="Edit Referral" class="btn btn-info pull-right">
                </div>
              </div>
            <?php echo form_close(); ?>
          <!-- /.box-body -->
